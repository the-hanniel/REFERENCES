...
$inleave = false;
$check = mysql_query("SELECT * FROM `tbl_leave` WHERE `UserID`='".$_SESSION['u_id']."' && `LeaveTo`>='".date('Y-m-d',time())."'")or die(mysql_error());
...
{	
	...
	$data = array("UserID"=>$_SESSION['u_id'],"LeaveFrom"=>"NOW()","LeaveTo"=>"NOW()","HOD"=>0,"HR"=>0,"Finance"=>0);
	if($db->InsertIfNotExist("tbl_leave",$data,array("UserID"=>$_SESSION['u_id'],"LeaveFrom"=>date("Y-m-d",time())),true)) {
		...
		if($db->InsertIfNotExist("leave_details",$data1,array("LeaveID"=>mysql_insert_id()),false)) {
			/* send the email to everyone that should be informed */
			/*
			*message body
			*/
			$emailbody = "I <b>";
			$emailbody .= $name . "</b><br>";
			$emailbody .= "I request for from ".date('Y-m-d');
			$emailbody .= " The requested is ".$leavetype;
			$emailbody .= "<br>My replacement while I will be on my leave is:<b>".$db->select1cell("tbl_users","Name",array("ID"=>$replacement),true)."</b> From <b>".$db->select1cell("tbl_users","Dept",array("ID"=>$replacement),true)." Department</b>";
			$emailbody .= "The address when I will be in the leave is <b>".$address."</b>";
			$emailbody .= "Thanks for regard";
			$emailtosend = array(
				"USERNAME"=>"ruberandindap@gmail.com",
				"PASSWORD"=>'PV1f$ael',
				"FROMADDRESS"=>$db->select1cell("tbl_users","Email",array("ID"=>$_SESSION['u_id']),true),
				"FROMNAME"=>$db->select1cell("tbl_users","Name",array("ID"=>$_SESSION['u_id']),true),
				"SUBJECT"=>"Request for leave from ".date('Y-m-d',time()),
				"MSG"=>$emailbody,
				"TOADDRESS"=>$db->select1cell("tbl_users","Email",array("Dept"=>"Human Ressource","UserType"=>3),true),
				"TONAME"=>$db->select1cell("tbl_users","Name",array("Dept"=>"HR","UserType"=>3),true),
			);
			/* get the sendNewEmail class */
			require_once "../../LIB/sendSMS.php";
			if(new SendNewEmail($emailtosend)) $successMsg = "Email is sent to ".$db->select1cell("tbl_users","Name",array("Dept"=>"HR","UserType"=>3),true);
			else $successMsg = "Data Saved!";
		} else {	
	...
	<div class="body">
		<div class="main_body">
			...
			<table width=100%>
				...
				<tr id=tableheader>
					<th>#</th><th>Leave Type</th><th>Start Date</th><th>End Date</th><th>HOD</th><th>HR</th><th>Finance</th>
				</tr>
				<?php
