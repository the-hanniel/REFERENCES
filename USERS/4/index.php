...
<div class="main">
	<?php
		...
		echo plotHeaderMenuInfo(basename(__FILE__),4,$db);
	?>
	<div class="body">
		<div class="main_body">
			<h2>Student Discipline Marks (Academic Year <?php echo $_SESSION['year'] ?>)</h2>
			<?php
				...
				#var_dump($_SESSION);
				$dept = $db->selectFields($tbl="tbl_dept",$field=array("ID","Acronym"),$condition=array("Acronym"=>array("Administration","GNC")),$limit=null,$order="ORDER BY Acronym ASC",$indexed=true,$sign='!=',$multiplereference=true,$distinct=true);
			...
			<form name=reference>
				...
				<table>
					<tr>
						<td>Department</td><td>Class</td><td>Term</td>
					</tr>
					<tr>
						<td>
							<select name=dept class=typeproform>
								...
								for($i=0;$i<count($dept);$i++){
									echo "<option onclick=\"studentMarksTeacher(document.reference.dept.value,document.reference.classid.value,document.reference.term.value,'');\" value='".$dept[$i]['ID']."'>".$dept[$i]['Acronym'];
								}
								?>
							</select>
						</td>
						<td>
							<select name=classid class=typeproform>
								<?php
								for($i=4;$i<=6;$i++){
									echo "<option onclick=\"studentMarksTeacher(document.reference.dept.value,document.reference.classid.value,document.reference.term.value,'');\">".$i;
								}
								?>
							</select>
						</td>
