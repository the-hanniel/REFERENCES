...
if($_GET['type'] == 1){
	...
	<select name=classid class=typeproform>
		...
		for($i=0;$i<count($dept);$i++){
			...
			echo "<option onclick='navigation(document.reference.dept.value,document.reference.classid.value,2,\"courseform\");studentMarksTeacherSitting(document.reference.dept.value,document.reference.classid.value,document.reference.courseid.value);' value='".$dept[$i]['Acronym'][3]."'>".$dept[$i]['Acronym'][3];
		}
		?>
	</select>
	<?php
...
if($_GET['type'] == 2){
	...
	<select name=courseid class=typeproforms>
		...
		for($i=0;$i<count($dept);$i++){
			...
			echo "<option value='".$dept[$i]['Acronym']."' onclick='studentMarksTeacherSitting(document.reference.dept.value,document.reference.classid.value,document.reference.courseid.value);'>".$dept[$i]['CourseName'];
		}
		?>
	</select>
	<?php
