...
<table border=1 width=80% style='border-top:4px solid #fff;border-right:4px solid #fff;border-bottom:4px solid #fff;'>
	<tr><th colspan=5><?php echo $db->select1cell("tbl_course","CourseName",array("Acronym"=>$_GET['acronym']),true); ?></th></tr>
	<tr id=tableheader>
		<th width=20px>#</th><th>Name</th><th width=60px>CAT/<?php echo $db->select1cell("tbl_course","Maximum",array("Acronym"=>$_GET['acronym']),true); ?> </th><th width=60px>Exam/<?php echo $db->select1cell("tbl_course","Maximum",array("Acronym"=>$_GET['acronym']),true); ?></th><th>&nbsp;</th>
	</tr>
	<?php
	...
	$data = $db->selectInMoreTable($lbl=array("tbl"=>array("school_report_db`.`tbl_student",$dbname."`.`tbl_student",$dbname."`.`tbl_marks"),"fld"=>array("school_report_db`.`tbl_student"=>array("ID","FirstName","LastName"),$dbname."`.`tbl_student"=>array("StudentID"),$dbname."`.`tbl_marks"=>array($field1,$field2)),"condition"=>array("school_report_db`.`tbl_student`.`ID"=>$dbname."`.`tbl_student`.`StudentID",$dbname."`.`tbl_marks`.`StudentID"=>$dbname."`.`tbl_student`.`StudentID",$dbname."`.`tbl_student`.`DeptID"=>$_GET['dept'],$dbname."`.`tbl_student`.`Class"=>$_GET['class'])),$multirows=true,$indexed=true, $order="ORDER BY `FirstName` ASC, `LastName` ASC");
	$out = "";
	if(count($data)<1){
		...
		for($i=$_GET['start'];$i<($_GET['end']>count($data)?count($data):$_GET['end']);$i++){
			...
			$out .= "</td>";
			$out .= "<td align=center><input id=s{$i} name=m{$data[$i]['ID']}1 onblur='saveMarks(studentid=\"".$data[$i]['ID']."\",field=\"{$field1}\",document.marks.m{$data[$i]['ID']}1.value,\"output".($i)."\",{$i})' type=text maxlength=6 onkeypress='return isNumberKey(event)' class=typeprofor value='".$data[$i][$field1]."'></td>";
