...
if(@$_POST['upload']){
	...
	/* the following code will read the data in the excel file and insert them into database */
		...
		$coursesindb = $db->selectFields($tbl="tbl_course",$field=array('CourseName',"Acronym"),$condition=null,$limit=null,$order="",$indexed=true,$sign='=',$multiplereference=false,$distinct=false);
		...
		foreach($validsheets as $id=>$name){
			...
			for ($row = 1; $row <= $highestRow; $row++){
				...
				$studentid = $db->selectFields($tbl="tbl_student",$field=array('ID'),$condition=array("FirstName"=>$rowData[0][1],"LastName"=>$rowData[0][2],"DeptID"=>$dept,"Class"=>$class),$limit=null,$order="",$indexed=true,$sign='=',$multiplereference=false,$distinct=false);
				//print_r($studentid);
				//echo "<br><br>";
...
<div class="main">
	...
	<div class="body">
		<div class="main_body">
			<h2>Second Sitting Marks (Academic Year <?php echo $_SESSION['year'] ?>)</h2>
			<?php
			...
			<form name=reference>
				...
				<table>
					<tr>
						<td>Department</td><td>Class</td><td>Course</td>
					</tr>
					<tr>
						<td>
							<select name=dept class=typeproform>
								...
								for($i=0;$i<count($dept);$i++){
									echo "<option onclick='navigation(document.reference.dept.value); navigation(document.reference.dept.value,document.reference.classid.value,2,\"courseform\");studentMarksTeacherSitting(document.reference.dept.value,document.reference.classid.value,\"\");' value='".$dept[$i]['DeptID']."'>".$db->select1cell("tbl_dept","Acronym",array("ID"=>$dept[$i]['DeptID']),true);
								}
								?>
							</select>
						</td>
						<td>
							<div id='classform'><script>navigation(document.reference.dept.value);</script></div>
						</td> <!--
						<td>
							<select name=term class=typeproform>
								<?php /*
								for($i=1;$i<=3;$i++){
									echo "<option onclick='studentMarksTeacherSitting(document.reference.dept.value,document.reference.classid.value,document.reference.term.value,document.reference.courseid.value);'>".$i;
								}
								//sleep(2); */
								?>
							</select>
						...
						<td id=course>
							<div id=courseform><script>alert("Course Could not be loaded, Retry");navigation(document.reference.dept.value,document.reference.classid.value,2,'courseform');</script></div>
						</td>
					</tr>
				</table>
			</form>
			<link rel="stylesheet" type="text/css" href="../../admin/css/tb.css" />
			<div id=studentmarks><script>studentMarksTeacherSitting(document.reference.dept.value,document.reference.classid.value,'');</script></div>
			<?php /*
			<table width=100%>
				<tr>
					<th colspan=7>
