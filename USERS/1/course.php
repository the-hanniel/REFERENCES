...
if(isset($_POST['submit']))
{
	...
	if($courseName ==	'' || $acronym == '' ) {
		...
		else
		{
			...
			$field = array();
			for($i=1;$i<=3;$i++) for($t=1;$t<=2;$t++)$field[] = array("NAME"=>$acronym.$i.$t,"TYPE"=>"FLOAT","LENGTH"=>null,"NOT_NULL"=>false,"DEFAULT"=>NULL);
...
<div class="main">
	...
	<div class="body">
		<div class="main_body">
			<h2 style="font-size:23px;">Course Management Page<!-- | <label><input type=checkbox name=newyear value='<?php// echo date('Y') ?>' onclick='newYear()' />Active Year<b> <?php// echo date('Y') ?></b></label>--></h2>
			<?php
			...
			<tr  style="font-size:16px;">
				...
				<td height = '30'>
					...
					?>
					<select name='dept' id='dets' class = 'typeproforms' onchange="loadTeacher(this.id, 'teacher')">
						<option disabled <?php if(!isset($_GET['dept'])) echo "selected"; ?> >Choose Department</option>
						<?php
						for($i=0;$i<count($dept);$i++) {
							echo "<option value='".$dept[$i]['ID']."' ".($dept[$i]['ID'] == $_GET['dept']?"selected":"").">"
								.$dept[$i]['DepartmentName'].($dept[$i]['Acronym'] != 'Administration'?"(".$dept[$i]['Acronym'].")":"")
								."</option>";
						};
						?>
					</select>
				...
				<td height = '30'>
					<select name=class class = 'typeproforms'>
						...
						if(@$_GET['sql'] == 'update.sql'){
							...
							?>
							<option <?php if(@$class[3] == 4) echo "selected" ?> >4
							<option <?php if(@$class[3] == 5) echo "selected" ?> >5
							<option <?php if(@$class[3] == 6) echo "selected" ?> >6
							<?php
						}
						?>
					</select>
			...
			<tr  style="font-size:16px;">
				...
				<td height = '30'>
					<script>
						function loadTeacher(s1, s2) {
							var s1 = document.getElementById(s1);
							self.location='course.php?dept=' + s1.value ;
						}
					</script>
					<?php
					if (isset($_GET['dept']))  $dpt = $_GET['dept'];
					$teacher = $db->selectFields($tbl='school_report_db`.`tbl_users',$field=array("ID","Name","Dept"),$condition=array("UserType"=>3, "Dept"=>$dpt),$limit=null,$order="ORDER BY Dept ASC, Name ASC",$indexed=true,$sign='=',false);
					?>
					<select name='teacher' id="teacher" class = 'typeproforms'>
						<?php
						for($i=0;$i<count($teacher);$i++) {
							echo "<option value='" . $teacher[$i]['ID'] . "' "
								. ($teacher[$i]['ID'] == $t?"selected":"") . ">" . $teacher[$i]['Name'] . "("
								. $db->select1cell("tbl_dept","Acronym",array("ID"=>$teacher[$i]['Dept']),true) . ")";
						}
						?>
					</select>
				</td>
			...
			<table width=100% border=1>
				<tr id=tableheader>
					<th>#</th><th>Course Name</th><!--<th>Acronym</th>--><th>Maximum</th><th>Class</th><th>Teacher</th><th colspan=3 width=10%>Action</th>
				</tr>
				<?php
				$data = $db->selectInMoreTable($lbl=array("tbl"=>array("school_report_db`.`tbl_users","tbl_course"),"fld"=>array("school_report_db`.`tbl_users"=>array("Name"),"tbl_course"=>array("ID","CourseName","Acronym","Maximum")),"condition"=>array("school_report_db`.`tbl_users`.`ID"=>"tbl_course`.`TeacherID")),$multirows=true,$indexed=true, $order="ORDER BY `tbl_course`.`Acronym` ASC, `school_report_db`.`tbl_users`.`Name` ASC");
				$out = "";
				for($i=0;$i<count($data);$i++){
					$out .= "<tr id=n".($i%2)."><td>";
					$out .= $i + 1;
					$out .= "</td><td>".$data[$i]['CourseName'];
					#$out .= "</td><td>".$data[$i]['Acronym']."</td>";
					...
					$out .= "<td align=right>".$data[$i]['Maximum']."</td>";
					$out .= "<td align=right>S".$data[$i]['Acronym'][3]." ".preg_replace('/'.substr($data[$i]['Acronym'],3).'/','',$data[$i]['Acronym'])."</td>";
					$out .= "<td>";
					$out .= $data[$i]['Name'];
					$out .= "</td>";
					$out .= "<td>";
