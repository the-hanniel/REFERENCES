	...
	<div class="body">
		<div class="main_body">
			...
			<table width=100%>
				...
				<tr id=tableheader>
					<th> # </th> <th> Name </th> <th> Leave Type </th> <th> Start Date </th> <th> End Date </th> <th> HOD </th> <th> Status </th>
				</tr>
				<?php
				$requestString = array("tbl"=>array("tbl_leave","leave_details","tbl_users"),"fld"=>array("tbl_users"=>array("Name"),"tbl_leave"=>array("ID","LeaveFrom","LeaveTo","HOD","HR","Finance"),"leave_details"=>array("Replacement","Details","Address")),"condition"=>array("tbl_users`.`Dept"=>$db->select1cell("tbl_users","Dept",array("ID"=>$_SESSION['u_id']),true),"tbl_leave`.`ID"=>"leave_details`.`LeaveID","tbl_leave`.`UserID"=>"tbl_users`.`ID","tbl_leave`.`HOD"=>1));
				...
				if(count($data) == 0) echo "<font color=red>No Request!</font>";
					...
					for($i=0;$i<count($data);$i++){
						...
						$out .= "<td>";
						$out .= $i + 1;
						$out .= "</td> <td>".$data[$i]['Name']."</td> <td>".$data[$i]['Details']."</td> <td align=center>".$data[$i]['LeaveFrom']."</td><td align=center>".$data[$i]['LeaveTo']."</td><td align=center>";
						$out .= $data[$i]['HOD']==1?"<font color=green>Approved</font>":"<abbr title='Approved'><a href='./?sql=allow.sql&id=".$data[$i]['ID']."' onclick='return confirmFunction(\"Do You Want To Approve The Leave For ".$data[$i]['Name']." \")'>Approve</a></abbr> | <abbr title='Dismiss'><a href='./?sql=dismiss.sql&id=".$data[$i]['ID']."' onclick='return confirmFunction(\"Do You Want To Dismiss The Leave For ".$data[$i]['Name']." \")'>Dismiss</a></abbr>";
