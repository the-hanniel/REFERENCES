...
if(@$_POST['update']){
	if($db->updatecells(array("Name"=>mysql_real_escape_string(trim($_POST['name'])),"Email"=>mysql_real_escape_string(trim($_POST['email']))),"tbl_users",array("UserName"=>mysql_real_escape_string(trim($_POST['uname']))))){
...
{	
	...
	if($hodName ==	'' || $uName == '' || $upass ==	'') {
		...
		$selectQuery			=	"SELECT * FROM `tbl_users` WHERE `UserName`='".$uName."' || `Dept`='Finance'";
		$existFlag				=	recordAlreadyExist($selectQuery);
		if($existFlag)
		{
			$errorMsg			=	"Error! User Name Already Exists Or Finance has a Director!";
		}	
		else
		{	
			$deptInsertQuery	= 	"INSERT INTO `tbl_users` (`ID`, `Name`,`Email`, `UserName`,`Password`,`UserType`,`Dept`,`ContractType`) VALUES (NULL, '{$hodName}','".$_POST['email']."', '{$uName}','".sha1($upass)."','4','Finance','".$_POST['userType']."')";
	...
	<div class="body">
		<div class="main_body">
			...
			<table width=100%>
				...
				<?php
				$data = $db->selectFields("tbl_users",array("ID","Name","Email","Dept","UserType"),array("UserType"=>4),null,"",true,'=');
				$out = "";
				for($i=0;$i<count($data);$i++){
					$out .= "<tr><td>";
					$out .= $i + 1;
					$out .= "</td><td>".$data[$i]['Name'];
					$out .= "</td><td>".$data[$i]['Email'];
					$out .= "</td><td align=center>".$data[$i]['Dept'];
					$out .= "</td><td align=center>".users($data[$i]['UserType']);
					$out .= "</td><td><a href='./finance.php?sql=reset.sql&id=".$data[$i]['ID']."' onclick='return confirmFunction(\"Reset Password for ".$data[$i]['Name']."?\")'>Reset</a>";
					$out .= "</td>";
					$out .= "<td>";
					$out .= "<a href='./finance.php?sql=update.sql&id=".$data[$i]['ID']."'>Update</a>";
					$out .= "</td>";
					$out .= "<td>";
					$out .= "<a href='./finance.php?sql=delete.sql&id=".$data[$i]['ID']."' onclick='return deleteFunction(\" ".$data[$i]['Name']."?\")'>Delete</a>";
					$out .= "</td>";
					$out .= "</tr>";
				}
				echo $out;
